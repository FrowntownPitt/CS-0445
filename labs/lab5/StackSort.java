...
public class StackSort {
    ...
    private static int[] doStackSort(int data[]) {
        ...
        for(int i:data){
            boolean added = false;
            while(!added){
                if(!upperValues.isEmpty() && upperValues.peek() < i){
                    lowerValues.push(upperValues.pop());
                } else if(!lowerValues.isEmpty() && lowerValues.peek() > i){
                    upperValues.push(lowerValues.pop());
                } else { //(upperValues.isEmpty() || upperValues.peek() >= i && (!lowerValues.isEmpty() && lowerValues.peek() <= i)){
                    upperValues.push(i);
                    added = true;
                }
            }
