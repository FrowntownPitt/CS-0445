...
// or null otherwise.
    ...
    public T remove() {
        ...
        for(int i=0; i<(int)(Math.random()*numberOfEntries); i++){
            scout = scout.next;
        }
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        if(numberOfEntries==0 && aBag.getCurrentSize()==0){
            result = true;
        } else if(numberOfEntries != aBag.getCurrentSize()){
            result = false;
        } else {
            Node scout = firstNode;

            for(int i=0; i<numberOfEntries; i++){
                if(aBag.getFrequencyOf(scout.data) == getFrequencyOf(scout.data)){
                    result = true;
                    scout = scout.next;
                } else {
                    result = false;
    ...
    public void removeDuplicates() {
        ...
        while(scout != null){
            if(getFrequencyOf(scout.data) > 1){
                remove(scout.data);
            } else {
                scout = scout.next;
            }
